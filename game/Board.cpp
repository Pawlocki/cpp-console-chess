...
void Board::setCurrentPlayer(int a)
{
    whoseTurn = a;
}
...
bool Board::isValidSquare(string check)
{
    if (check.length() != 2)
        return false;
...
bool Board::isCurrentPlayersPiece(string piece)
{
...
bool Board::isOCCupiedByEnemyPiece(string check)
{
...
bool Board::isOccupiedByOwnPiece(string check)
{
    string piece = getPieceFromSquare(check);
...
bool Board::isNotOccupied(string check)
{
    string piece = getPieceFromSquare(check);
...
int Board::convertColumnToNumber(string check)
{
    int converter = -1;
...
int Board::convertRowtoNumber(string check)
{
    return 8 - (check[1] - '0');
}
string Board::convertPositionToSquare(int row, int column)
{
    char file = 'a' + column;
    char rank = '8' - row;
...
void Board::clearLastPawnPushTableWhite()
{
...
void Board::clearLastPawnPushTableBlack()
{
    for (int row = 0; row < 8; row++)
    {
        for (int column = 0; column < 8; column++)
...
string Board::getPiece(int row, int column)
{
...
void Board::setPiece(int row, int column, string piece)
{
    if (row >= 0 && row < 8 && column >= 0 && column < 8)
        board[row][column] = piece;
}
string Board::getPieceFromSquare(string square)
{
...
void Board::setPieceOnSquare(string square, string piece)
{
    int row = convertRowtoNumber(square);
    int column = convertColumnToNumber(square);
...
MoveResult Board::tryMovePiece(string fromSquare, string toSquare, bool shouldMove, string promotionPiece)
{
    ...
    {
        return {false, MoveError::InvalidPieceMovement};
    }
    ...
    {
        return {false, MoveError::CannotCaptureKing};
    }
    ...
    if (isSquareAttacked(findKingPosition()) == true)
    {
        ...
        return {false, MoveError::KingWouldBeInCheck};
    }
    ...
    if ((movingPiece == "P" && toRow == 0) || (movingPiece == "p" && toRow == 7))
        promotePawn(toSquare, promotionPiece);

    return {true, MoveError::None};
}
bool Board::isPawnMovementValid(string fromSquare, string toSquare)
{
    int fromColumn = convertColumnToNumber(fromSquare);
    int fromRow = convertRowtoNumber(fromSquare);
...
bool Board::isRookMovementValid(string fromSquare, string toSquare)
{
    int fromColumn = convertColumnToNumber(fromSquare);
    int fromRow = convertRowtoNumber(fromSquare);
...
bool Board::isBishopMovementValid(string fromSquare, string toSquare)
{
    int fromColumn = convertColumnToNumber(fromSquare);
    int fromRow = convertRowtoNumber(fromSquare);
...
bool Board::isKnightMovementValid(string fromSquare, string toSquare)
{
    int fromColumn = convertColumnToNumber(fromSquare);
    int fromRow = convertRowtoNumber(fromSquare);
...
bool Board::isQueenMovementValid(string fromSquare, string toSquare)
{
...
bool Board::isKingMovementValid(string fromSquare, string toSquare)
{
    int fromColumn = convertColumnToNumber(fromSquare);
    int fromRow = convertRowtoNumber(fromSquare);
...
bool Board::isCastlingMoveValid(string fromSquare, string toSquare)
{
    int fromColumn = convertColumnToNumber(fromSquare);
    int fromRow = convertRowtoNumber(fromSquare);
...
void Board::promotePawn(string square, string promotionPiece)
{
    if (promotionPiece == "")
    {
        if (getCurrentPlayer() == 0)
    ...
    setPieceOnSquare(square, promotionPiece);
}
bool Board::isPawnPromotionMove(string fromSquare, string toSquare)
{
    if (isValidSquare(fromSquare) == false || isValidSquare(toSquare) == false)
        return false;

    string piece = getPieceFromSquare(fromSquare);
    int toRow = convertRowtoNumber(toSquare);
...
bool Board::isSquareAttackedByPawn(string squareToCheck)
{
...
bool Board::isSquareAttackedByRook(string squareToCheck)
{
    int squareColumn = convertColumnToNumber(squareToCheck);
    int squareRow = convertRowtoNumber(squareToCheck);
...
bool Board::isSquareAttackedByBishop(string squareToCheck)
{
    int squareColumn = convertColumnToNumber(squareToCheck);
    int squareRow = convertRowtoNumber(squareToCheck);
...
bool Board::isSquareAttackedByKnight(string squareToCheck)
{
    int squareColumn = convertColumnToNumber(squareToCheck);
    int squareRow = convertRowtoNumber(squareToCheck);
...
bool Board::isSquareAttackedByQueen(string squareToCheck)
{
    int squareColumn = convertColumnToNumber(squareToCheck);
    int squareRow = convertRowtoNumber(squareToCheck);
...
bool Board::isSquareAttackedByKing(string squareToCheck)
{
    int squareColumn = convertColumnToNumber(squareToCheck);
    int squareRow = convertRowtoNumber(squareToCheck);
...
bool Board::isSquareAttacked(string squareToCheck)
{
...
bool Board::playerHasAnyLegalMove()
{
    for (int row = 0; row < 8; row++)
    {
        for (int column = 0; column < 8; column++)
            ...
            if (isCurrentPlayersPiece(piece) == true)
            {
                ...
                for (int targetRow = 0; targetRow < 8; targetRow++)
                {
                    for (int targetColumn = 0; targetColumn < 8; targetColumn++)
                    {
                        ...
                        MoveResult moveResult = tryMovePiece(fromSquare, toSquare, false);

                        if (moveResult.wasMoveDone == true)
                            return true;
                    }
                }
            }
